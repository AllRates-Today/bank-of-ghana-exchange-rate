# Bank of Ghana Exchange Rates API — bank-of-ghana-exchange-rate

[![npm version](https://img.shields.io/npm/v/bank-of-ghana-exchange-rate.svg)](https://www.npmjs.com/package/bank-of-ghana-exchange-rate)
...
- 🏛️ **Official published rates** — Bank of Ghana's own table, with the publisher's own `rate_date` on every response
- 📅 **History back to 2007** — point-in-time tables and daily series for any past date
- 🔀 **Published vs derived, always flagged** — computed inverse/cross pairs carry `derived: true`, never mixed with official prints
- ⚡ **Zero dependencies** — pure ESM + CJS over global `fetch`; Node 18+, Bun, Deno, and edge runtimes
- 🔷 **Type-safe** — full TypeScript definitions shipped with the package
- 🧾 **Compliance-grade metadata** — `rate_type`, publication date, and source disclaimer on every response

> **Official rate, not mid-market:** every value here is a number Bank of Ghana itself published, fixed once printed and carrying the central bank's own `rate_date` — what filings and audits require. Need the live interbank midpoint for pricing or display instead? Use the [mid-market API](https://allratestoday.com/docs/) or [`@allratestoday/sdk`](https://www.npmjs.com/package/@allratestoday/sdk). The two can diverge by several percent.

## ⚡ Try it without a key

The latest Bank of Ghana table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/bog?source=USD&target=GHS"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/bog').then((x) => x.json());
...
```bash
npm install bank-of-ghana-exchange-rate
```

```bash
yarn add bank-of-ghana-exchange-rate
```

```bash
pnpm add bank-of-ghana-exchange-rate
```

Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/bank-of-ghana-exchange-rate`](https://www.npmjs.com/package/@allratestoday/bank-of-ghana-exchange-rate) — same code, same versions.

## 🏁 Quick start

```js
import { getRate } from 'bank-of-ghana-exchange-rate';

const pair = await getRate('USD', 'GHS', { apiKey: 'art_live_...' });
console.log(pair.rate, pair.rate_date); // the official Bank of Ghana rate, on the central bank's own date
```
...
```js
import { getLatestRates } from 'bank-of-ghana-exchange-rate';
...
Paid plans. The official table for any date since 2007 — weekends and holidays return the most recent published date, flagged via `published_on_requested_date`, which is exactly the in-force rate a filing needs.

```js
import { getRatesForDate } from 'bank-of-ghana-exchange-rate';

const day = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...' });
// Optionally narrow to one pair:
const one = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...', source: 'USD', target: 'GHS' });
```
...
The Bank of Ghana publishes daily interbank foreign exchange rates — buying, selling, and mid rates for the cedi against 14 currencies, computed from actual interbank trades. They are the official reference used by Ghanaian banks, the revenue authority, and businesses pricing in cedis.

- Publisher's own page: [Daily interbank FX rates](https://www.bog.gov.gh/treasury-and-the-markets/daily-interbank-fx-rates/) · [www.bog.gov.gh](https://www.bog.gov.gh)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Bank of Ghana rates page](https://allratestoday.com/central-bank-rates-api/bog/)
...
- Bank of Ghana quotes **GHS per 1 unit of foreign currency** (e.g. `base: "USD", quote: "GHS"` means GHS per one US dollar).
...
Loading the official Bank of Ghana rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/bog/latest?format=xml&api_key=art_live_..."
```
...
If Bank of Ghana does not print a pair directly, the API resolves it from the central bank's own table and says so — official and computed values are never confused:

| `method` | `derived` | Meaning |
| --- | --- | --- |
| `published` | `false` | The central bank printed this pair directly |
| `inverse` | `true` | Computed as 1 ÷ the published opposite direction |
| `cross` | `true` | Computed via GHS from two published rates |
...
| `404` | Pair or date range not covered by Bank of Ghana |
...
```ts
import type { LatestRates, PairRate, DatedRates, RateEntry, HistoryQuery, RequestOptions } from 'bank-of-ghana-exchange-rate';
```

## 📦 CommonJS

```javascript
const { getRate } = require('bank-of-ghana-exchange-rate');

getRate('USD', 'GHS', { apiKey: 'art_live_...' }).then((pair) => console.log(pair.rate));
```
...
- [Bank of Ghana rates page](https://allratestoday.com/central-bank-rates-api/bog/) — live table, publication cadence, FAQ
- [All central bank sources](https://allratestoday.com/central-bank-rates-api/)
- [Package docs on the site](https://allratestoday.com/docs/sdk/bank-of-ghana-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
...
- [GitHub](https://github.com/AllRates-Today/bank-of-ghana-exchange-rate)
